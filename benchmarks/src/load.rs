...
/// and returns its p50 in the second tuple element; otherwise `None`.
pub async fn run_cell(
    ...
    window: Duration,
    streaming: bool,
) -> (crate::stats::Summary, Option<f64>) {
    ...
    for _ in 0..concurrency {
        let (url, recording, stop, latencies, ttfbs) = (
            url.clone(),
            recording.clone(),
            stop.clone(),
            latencies.clone(),
        ...
        workers.push(tokio::spawn(async move {
            ...
            while !stop.load(Ordering::Relaxed) {
                ...
                let Ok(resp) = client.post(&url).body(body).send().await else {
                    continue;
                };
...
mod tests {
    ...
    async fn run_cell_collects_samples_and_positive_rps() {
        ...
        .await;
        assert!(summary.count > 0, "no samples collected");
