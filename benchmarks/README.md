...
Each scenario is measured twice, once direct to the upstream (baseline)
and once through sluice, and the numbers reported are the *added* latency.
...
chart and JSON, so treat cross-machine comparisons as directional only.
...
- **translation**: openai-to-anthropic wire translation
...
after a 2s warmup. Not run in CI, since shared-runner numbers are noise.
